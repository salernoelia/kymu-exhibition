<template>
    <div class="h-full w-full">
        <div class="flex flex-col w-full h-full items-center justify-start gap-8 pt-20">
            ...
            <DotLottieVue class="absolute w-full h-full bottom-[-25px] left-0 z-[-1] overflow-hidden" autoplay loop
                src="/lottifiles/success.lottie" />

            <div class="flex flex-row justify-center items-center gap-16">
                <div v-for="e in exerciseStore.exercises" :key="e.id"
                    ...
                    <h3>{{ e.name }}</h3>
                    <p v-if="e.type === 'p5_game' && e.results.achieved_accuracy" class="text-sm">
        ...
        <KeyInstruction button="ok" action="restart" />
    </div>
</template>
